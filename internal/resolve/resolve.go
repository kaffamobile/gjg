// Package resolve provides path resolution utilities for Java executables and JAR files.
package resolve
...
func ResolveJava(javaDir, javaExe, exeDir string) (string, error) {
	exeName := javaExe
...
func ResolveJar(jar, exeDir string) (string, error) {
	p := jar
	if !filepath.IsAbs(p) {
